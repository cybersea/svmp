...
class Site(object):
    ...
    """
    def __init__(self,id,zmArea,zmAreaVar,aj=None,conversion=None):
        # is this less kludgy?
        self.id = id
        self.zmArea = zmArea
...
class BaseStratum(object):
    ...
    Attributes:
    geo -- geomorph stratum (flats, wide fringe, fringe)
    samp -- sampling stratum (core, persistent, rotational)
    analysis -- analysis stratum (core, persistent flats, flats, fringe, wide fringe)
    extrapolation -- extrapolation stratum (none, area, linear)
...
class FlatsStratum(BaseStratum):
    ...
    Attributes:
    Ni -- count of all sites in the stratum (from base data, flats.shp)
    A2 -- sum of total stratum area (from base data, flats.shp), in square meters
    Aij -- sum of area sampled, for sites in the Sample (from base data, flats.shp) 
    
    """
    def __init__(self,analysis,extrapolation,stratum_fc,gp,conversion=None):
        #self.Ni = Ni
        #self._A2 = A2
        #self._Aij = Aij
...
class FringeStratum(BaseStratum):
    ...
    Attributes: 
    Ni -- count of all sites in the stratum (from base data, fringe.shp)
    LT -- sum of total stratum length (from base data, fringe.shp), in meters
    LN -- length of sampling frame, in meters
    ...
    @property
    def LN(self):
        return self.Ni * 1000 / svmpUtils.sf_m
    ...
    def importSites(self,sites_list):
        ...
        for s in sites_list:
            ...
            if len(s) == 4:
                ...
                aj = None
            kwargs = {'aj':aj,'conversion':self.conversion}
